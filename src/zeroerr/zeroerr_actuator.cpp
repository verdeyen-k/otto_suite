...
void configure_zeroerr_pdos(ethercat::SoemMaster &master, int slave_index) {
    master.set_config_func(slave_index, [](ethercat::SoemMaster &m, int idx) {
        const std::vector<ethercat::PdoMapEntry> rx_entries = {
            {0x607A, 0, 32},  // Target position (unused in CSV mode, mapped for completeness)
...
void ZeroErrActuator::update() {
    ...
    // Safe default is zero, not "hold current position" -- forced to zero
    // whenever not actively enabling, same reasoning as CopleyAxis, so a
    // stale nonzero command from a previous run can never be re-applied
    // by accident on a later enable. The position loop is closed by the
    // caller (see this class's header comment); this is a plain velocity
    // actuator.
